% November, 2017
% script for processing for test with c1 and e=0 - PRO
clear all; clc;
addpath('../../../../');
...
stencil_size(3,:)=[1 2 3 4 5];% point_value stencil size
    ...
    for d=1:numel(degree)
        for f=1:numel(flux_num)
            ...
            for k=1:numel(num_cells)
                ...
                domain.point(2).coord=domain.point(2).coord;
                model_test1;
                ...
                pro{j}{d}{k,f}=mfvl_pro_cdr_c1_15_11_17(mesh,domain,mod,degree(d),flux,[weight1 weight2],stencil_size(:,d),auto_stencil_opt);
