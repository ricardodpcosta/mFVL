...
weight2=1;
scheme='cdr';
auto_stencil_opt='auto';
stencil_size(1,:)=[2 3 4 5 6];
stencil_size(2,:)=zeros([1 numel(degree)]);%mean_value stencil%mean_value stencil
stencil_size(3,:)=[2 3 4 5 6];% point_value stencil size
stencil_size(4,:)=[2 3 4 5 6];% none stencil size
    ...
    for d=1:numel(degree)
        for f=1:numel(flux_num)
            ...
            for k=1:numel(num_cells)
                ...
                domain.point(2).coord=domain.point(2).coord+((1/num_cells(k))^2)/2;
                model_test1;
                ...
                pro{j}{d}{k,f}=mfvl_pro_cdr_c3_15_11_17(mesh,domain,mod,degree(d),flux,[weight1 weight2],stencil_size(:,d),auto_stencil_opt);
