...
stencil_size(2,:)=zeros([1 numel(degree)]);%mean_value stencil%mean_value stencil%mean_value stencil
...
save('output/requisites.mat','num_cells','degree','weight1','weight2','flux_num','stencil_size','auto_stencil_opt');
for d=1:numel(degree)
    for f=1:numel(flux_num)
	    ...
	    disp([d f k]);
            ...
            pro{d}{k,f}=mfvl_pro_cdr(m{k},domain,mod,degree(d),flux,[weight1 weight2],stencil_size(:,d),auto_stencil_opt);
            
            pro_s=pro{d}{k,f};
            sol=pro{d}{k,f}.u_approx;
