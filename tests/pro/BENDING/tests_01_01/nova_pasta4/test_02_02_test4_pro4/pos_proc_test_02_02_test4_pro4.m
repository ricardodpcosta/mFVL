...
mfvl_lib;
func=@(x)-exp(x)+2*(exp(1)-1)*x+2; % attention
...
p10_pro4=format_errors_orders(error{1,1},numel(num_cells),num_cells);
p10_pro4_l1=format_errors_orders(error{2,1},numel(num_cells),num_cells);

p11_pro4=format_errors_orders(error{1,2},numel(num_cells),num_cells);
p11_pro4_l1=format_errors_orders(error{2,2},numel(num_cells),num_cells);

p12_pro4=format_errors_orders(error{1,3},numel(num_cells),num_cells);
p12_pro4_l1=format_errors_orders(error{2,3},numel(num_cells),num_cells);

save('output/test_pro_bending_02_02_test4_pro4_results','p10_pro4','p10_pro4_l1','p11_pro4','p11_pro4_l1','p12_pro4','p12_pro4_l1');
